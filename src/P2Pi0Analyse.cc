...
pulls_4g = new HistoManu2("pulls_4g","pulls_4g", 200, -10, 10, 3, 0, 3);
pulls_proton = new HistoManu2("pulls_proton","pulls_proton", 200, -10, 10, 3, 0, 3);
...
thetaproton_fit_vs_rek= new HistoManu("thetaproton_fit_vs_rek", "Polar angle (Signal); #theta_{rek}-#theta_{meas} [deg]", 400,-200,200);
...
cosverteilung_collerated_proton = new HistoManu2("cosverteilung_collerated_proton", "Cos-Verteilung; cos(#theta_{2#pi});E_{beam} [MeV]", 72,-1,1,37,200,1421);
...
h_energy_sum_2pi0_5ped = new HistoManu("h_energy_sum_2pi0_5ped ", "h_energy_sum_2pi0_5ped ",  300, 0, 1557);

h_energy_taps_2pi0_5ped = new HistoManu2("h_energy_taps_2pi0_5ped ", "h_energy_taps_2pi0_5ped ",  900, 0, 900,300,0,1557);



h_energy_sum_2pi0_5ped_weightcos2pi0 = new HistoManu("h_energy_sum_2pi0_5ped_weightcos2pi0 ", "h_energy_sum_2pi0_5ped_weightcos2pi0 ",  300, 0, 1557);

h_energy_sum_2pi0_5ped_weightcosppi0 = new HistoManu("h_energy_sum_2pi0_5ped_weightcosppi0 ", "h_energy_sum_2pi0_5ped_weightcosppi0 ",  300, 0, 1557);
...
//reconstructed
cos2pi0_beamphoton_energysum_rek = new HistoManu3("cos2pi0_beamphoton_energysum_rek", "cos2pi0_beamphoton_energysum_rek;cos #theta_{2#pi};E_{#gamma} [MeV];E_{sum} [MeV]",  18, -1, 1,30, 220, 1420 ,200, 0, 2000);
...
// ................................invarianteMasse Cut...................................................................................
		...
		// 	__________________Coplanarity______________________________________________________
			...
			if((missingmass>(unten_mass) && missingmass<(oben_mass))&& (((invariantemasse12 > unten_inv && invariantemasse12 < oben_inv) && (invariantemasse34 > unten_inv && invariantemasse34 < oben_inv)) || ((invariantemasse13 > unten_inv && invariantemasse13 < oben_inv) && (invariantemasse24 > unten_inv && invariantemasse24 < oben_inv)) || ((invariantemasse14 > unten_inv && invariantemasse14 < oben_inv) && (invariantemasse23 > unten_inv && invariantemasse23 < oben_inv)))){
				...
				fitter.Set(photon1_4vektor, photon2_4vektor, photon3_4vektor, photon4_4vektor);
				fitter.SetProton(proton_4vect_meas);
				fitter.SetBeam(beamphoton1E);

				if(!fitter.Solve()) continue;
				Double_t    cl  = TMath::Prob(fitter.GetFittedResult().ChiSquare,18-fitter.GetFittedResult().NDoF);
				CL->Fill(cl);
				//PULLS
				for(int i=0; i<4; i++)
				{
					for(int p=0; p<3; p++)
					{
						...
						s << "Ph" << i;
						s << "[" << p << "]";
						const APLCON::Result_Variable_t& var = fitter.GetFittedResult().Variables.at(s.str());
						pulls_4g->Fill(var.Pull, p, time);
					}
				}
				...
				for(Int_t p=0;p<3;p++){
					...
					std::stringstream s;
					s << "Be[" << p << "]";
					const APLCON::Result_Variable_t& var_beam = fitter.GetFittedResult().Variables.at(s.str());
					pulls_beam->Fill(var_beam.Pull,p,time);
				}
				...
				for(Int_t p=0;p<2;p++){
					...
					std::stringstream s;
					s << "PA[" << p << "]";
					const APLCON::Result_Variable_t& var_proton = fitter.GetFittedResult().Variables.at(s.str());
					//cout << var_proton.Value.After << endl;
					pulls_proton->Fill(var_proton.Pull,p,time);
				}
				...
				if(thetadiff > (unten_theta) && thetadiff < (oben_theta)){
					//Phi-Difference
					 anglephiproton_fit= TMath::RadToDeg()*fitter.GetFittedResult().Variables.at("PA[1]").Value.After;
					Double_t phi_fit = phimeson - anglephiproton_fit;
				...
				if(phi > (unten_copl) && phi < (oben_copl)){
					//theta differenz
					anglethetaproton_fit = TMath::RadToDeg()*fitter.GetFittedResult().Variables.at("PA[0]").Value.After;
					Double_t theta_fit = anglethetaproton_meas-anglethetaproton_fit;
					thetaproton_fit_vs_rek->Fill(theta_fit,time);
				}
			...
			//	cout << "\t" << GetRootinos()->GetNParticles() << "\t" << fitter.Solve() << "\t" << anglethetaproton_fit << "\t" << anglethetaproton_meas << "\t" << TMath::RadToDeg()*fitter.GetFittedResult().Variables.at("PA[0]").Value.Before << endl;
				...
				cout << "m12: " << (fitter.GetFittedGamma(0)+fitter.GetFittedGamma(1)).M() << "\t" << invariantemasse12<< "\t" << cl <<endl;
				cout << "m34:  " <<(fitter.GetFittedGamma(2)+fitter.GetFittedGamma(3)).M() << "\t" << invariantemasse34 << "\t" << cl << endl;
				cout << "m13: " << (fitter.GetFittedGamma(0)+fitter.GetFittedGamma(2)).M() << "\t" << invariantemasse13 << "\t" << cl << endl;
				cout << "m24: " << (fitter.GetFittedGamma(1)+fitter.GetFittedGamma(3)).M() << "\t" << invariantemasse24 << "\t" << cl << endl;
				cout << "m14: " << (fitter.GetFittedGamma(0)+fitter.GetFittedGamma(3)).M() << "\t" << invariantemasse14 << "\t" << cl << endl;
				cout << "m23: " << (fitter.GetFittedGamma(1)+fitter.GetFittedGamma(2)).M() << "\t" << invariantemasse23 << "\t" << cl << endl;
				...
				if(cbtrigger=kTRUE || GetScalers()->GetNEntries()==0){
					...
					cos2pi0_beamphoton_energysum_rek->Fill(cospi0pi0,beamphoton1E,GetTrigger()->GetEnergySum(),time);

...
int main(int argc, char *argv[])
{
    ...
    // Create instance of P2Pi0Analyse.h class
    P2Pi0Analyse* ppi0 = new P2Pi0Analyse;
    ...
    {
        cout << "ERROR: P2Pi0Analyse.h Init failed!" << endl;
        return 0;
    }
    ...
    if(file_in.length() > 0)
    {
        ...
        if(length >= 5)
        {
            ...
            if(!ppi0->StartFile(file_in.c_str(), file_out.c_str())) cout << "ERROR: P2Pi0Analyse.h failed on file " << file_in << "!" << endl;
            files_found++;
        }
    }
            ...
            if(length >= 5)
            {
                ...
                if(((strcmp(prefix.c_str(),pre_in.c_str()) == 0)) && (strcmp(suffix.c_str(),".root") == 0))
                {
                    ...
                    if(!ppi0->StartFile(file_in.c_str(), file_out.c_str()))
                        cout << "ERROR: P2Pi0Analyse.h failed on file " << file_in << "!" << endl;
